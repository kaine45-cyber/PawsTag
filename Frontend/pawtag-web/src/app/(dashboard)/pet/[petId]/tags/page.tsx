...
const FALLBACK_AVATAR = "https://images.unsplash.com/photo-1543466835-00a7907e9de1?w=120&q=80";
  ...
  if (!pet) {
    return (
      <div className="flex flex-col items-center justify-center min-h-full gap-4 px-5">
        ...
        <p className="text-[17px] font-extrabold text-[#1A2332] font-display text-center">Pet not found</p>
        <Link
          href={ROUTES.petList}
  ...
  return (
    <div className="flex flex-col min-h-full px-5 pt-6 pb-8 gap-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-[22px] font-black text-[#1A2332] font-display">Smart Tag</h1>
      ...
      <div className="bg-white rounded-2xl shadow-card-md overflow-hidden">
        <div className="px-4 py-3 border-b border-[rgba(74,143,232,0.1)] flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[14px] font-extrabold text-[#1A2332] font-display">QR Code — {pet.name}</p>
          </div>
          <span className="text-[10px] font-bold text-[#22C55E] bg-[#EDF7F2] px-2.5 py-1 rounded-full font-body">Active</span>
        </div>
        ...
        <div className="p-6 flex flex-col items-center gap-4">
          ...
          <p className="text-[11px] text-[#9BAABB] font-body">
            Tag ID: <span className="font-mono font-medium text-[#1A2332]">PT-2025-{pet.name.toUpperCase()}-001</span>
          </p>
      ...
      <div className="bg-white rounded-2xl shadow-card overflow-hidden">
        <div className="px-4 py-3 border-b border-[rgba(74,143,232,0.1)] flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-[14px] font-extrabold text-[#1A2332] font-display">NFC Tag</p>
          </div>
          <button
            type="button"
            onClick={toggleNfc}
        ...
        <div className="p-4 flex flex-col gap-3">
          <div className="flex items-center gap-3 bg-[#FFF7F0] rounded-2xl px-4 py-3">
            ...
            <p className="text-[12px] text-[#6B7A8D] font-body leading-relaxed">
              NFC allows any smartphone to tap {pet.name}&apos;s tag and instantly open the profile — no app required.
            </p>
          </div>

          <div>
            <label className="block text-[12px] font-semibold text-[#1A2332] font-body mb-1">NFC Link</label>
          ...
          <div className={`rounded-2xl px-4 py-3 flex items-center gap-2 ${nfcOn ? "bg-[#EDF7F2]" : "bg-[#EEF2F7]"}`}>
            ...
            <p className={`text-[12px] font-semibold font-body ${nfcOn ? "text-[#2A6B47]" : "text-[#9BAABB]"}`}>
              {nfcOn ? `NFC enabled — tap any NFC phone to ${pet.name}'s tag` : "NFC disabled"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-card p-4">
        <p className="text-[14px] font-extrabold text-[#1A2332] font-display mb-1">Activate a Physical Tag</p>
        <p className="text-[12px] text-[#9BAABB] font-body mb-3">Got a PawsTag sticker? Enter its code to link it to {pet.name}.</p>
      ...
      {/* Collar Preview */}
      <div className="bg-white rounded-2xl shadow-card p-4">
        <p className="text-[14px] font-extrabold text-[#1A2332] font-display mb-3">Collar Preview</p>
